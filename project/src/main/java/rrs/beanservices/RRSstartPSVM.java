...
public class RRSstartPSVM {
	...
	public static void main(String[] args)throws IOException  
	{
		...
		try
		{
			...
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/railway?createDatabaseIfNotExist=true","root","RKu99");
			
			
			PreparedStatement st=con.prepareStatement("create table if not exists login(email varchar(255),name varchar(255) not null,password varchar(255) not null, usertype varchar(255) not null, primary key(email))");
